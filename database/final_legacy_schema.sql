-- Updated Legacy System Database Schema
-- This schema matches the Flutter code expectations with year and month columns

-- Legacy folders table (updated with year and month)
CREATE TABLE legacy_folders (
    ...
    status TEXT DEFAULT 'draft' CHECK (status IN ('draft', 'completed', 'archived', 'deleted')),
...
-- Legacy files table (references folders)
...
CREATE POLICY "Users can view non-deleted legacy folders" ON legacy_folders
...
CREATE POLICY "Users can view non-deleted legacy files" ON legacy_files
    FOR SELECT USING (status != 'deleted');
...
BEGIN
    ...
    SET 
        ...
        archived_at = NOW(),
        archived_by = user_id
    WHERE id = folder_id AND status IN ('draft', 'completed');
...
BEGIN
    ...
    SET 
        ...
        deleted_at = NOW(),
        deleted_by = user_id
    WHERE id = folder_id AND status IN ('draft', 'completed', 'archived');
...
SELECT * FROM legacy_folders WHERE status IN ('draft', 'completed');
